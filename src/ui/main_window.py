...
class MainWindow:
    ...
    def _init_dropdown(self):
        ...
        self._dropdown = customtkinter.CTkOptionMenu(master=self._frame, values=choices)
    ...
    def _init_entry_field(self):
        ...
        self._entry = customtkinter.CTkEntry(master=self._frame, placeholder_text="Input in format 10 or 10.00", textvariable=self._entry_value)
        self._entry.grid(padx=5, pady=5, sticky=constants.EW)

    def _init_submit_button(self):
        self._submit_button = customtkinter.CTkButton(master=self._frame, corner_radius=20, text="Submit", command=self.submit_value)
        self._submit_button.grid(padx=5, pady=5, sticky=constants.EW)

    def _init_info_labels(self):
        self._initial_budget_label = ttk.Label(self._frame, text=f"Initial Budget: {self.initial_budget:.2f}")
        self._current_budget_label = ttk.Label(self._frame, text=f"Current Budget: {self.current_budget:.2f}")
        self._total_expense_label = ttk.Label(self._frame, text=f"Total Expenses: {self.total_expense:.2f}")
        self._total_income_label = ttk.Label(self._frame, text=f"Total Income: {self.total_income:.2f}")
    ...
    def _update_info_labels(self):
        ...
        self.current_budget = self.initial_budget + self.total_income - self.total_expense

        self._initial_budget_label.config(text=f"Initial Budget: {self.initial_budget:.2f}")
        self._current_budget_label.config(text=f"Current Budget: {self.current_budget:.2f}")
        self._total_expense_label.config(text=f"Total Expenses: {self.total_expense:.2f}")
        self._total_income_label.config(text=f"Total Income: {self.total_income:.2f}")
    ...
    def _init_history_section(self):
        ...
        for i, row in enumerate(self.history):
            ...
            label = ttk.Label(self._history_frame, text=f"{timestamp}: {transaction_type} - {amount:.2f}")
